...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		// Tests the grey scale of an image:
	...
	public static Color[][] read(String fileName) {
		...
		for (int i = 0; i < image.length; i++){ //for every column
			for (int j = 0; j < image[i].length; j++){ //for every ij space 
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++){
			for (int j = 0; j < image[i].length; j++){
				imageNew[i][j] = image[i][image[0].length - 1 - j];
			}
		}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int i = 0; i < image.length; i++){
			for (int j = 0; j < image[i].length; j++){
				imageNew[i][j] = image[image.length - 1 - i][j];
			}
		}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i = 0; i <= n; i++) {
			double alpha = (double) (n - i) / n; // Corrected the blending factor calculation
